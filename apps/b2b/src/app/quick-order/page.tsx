...
const BASE_PRODUCTS = [
  {
    ...
    expiry: "09/2030",
    image:
      "https://cdnv2.tgdd.vn/mwg-static/ankhang/Products/Images/4611/334731/vien-uong-tranh-thai-new-choice-new-choice-25-vi-x-28-vien-thumb01-638767909191018593-600x600.jpg",
    qty: 0,
  ...
  },
  {
    ...
    expiry: "12/2026",
    image:
      "https://cdnv2.tgdd.vn/mwg-static/ankhang/Products/Images/10245/327225/cao-dan-salonsip-gel-patch-hop-8-bao-3-mieng-thumb-638792755913157852-600x600.jpg",
    qty: 5,
  ...
  },
  {
    ...
    expiry: "01/2025",
    image:
      "https://cdn.tgdd.vn/Products/Images/9921/218987/dau-gio-nau-pharmedic-3ml-thumb01-600x600.jpg",
    qty: 0,
    isLowStock: true,
  ...
  },
  {
    ...
    expiry: "10/2028",
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTejcjXCkTh8ncMSToEfQM_4AjhRc6x0MT3zg&s",
    qty: 10,
...
const VOUCHERS = [
  ...
  },
  {
    ...
    desc: "Tối đa 30.000đ, cho đơn từ 1.000.000đ",
    discount: 30000,
    minOrder: 1000000,
  },
];
    ...
    <div className="min-h-screen bg-slate-50 pb-32 lg:pb-24 flex flex-col font-sans relative selection:bg-orange-200">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm shrink-0">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <Link
            href="/"
            className="w-10 h-10 shrink-0 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-full flex items-center justify-center transition-colors"
          >
            <ChevronLeft size={24} />
          </Link>
          <h1 className="hidden md:block text-xl font-black text-slate-800 tracking-tight mr-4">
            Đặt hàng nhanh
          </h1>
          <div className="flex gap-2 flex-1 max-w-3xl">
            <div className="relative flex-1 group">
              ...
              <input
                type="text"
                placeholder="Gõ tên, SKU, hoạt chất..."
                className="w-full pl-9 pr-10 py-2.5 md:py-3 bg-slate-100 border border-transparent focus:bg-white focus:border-orange-500 rounded-xl text-sm font-medium outline-none transition-all placeholder:text-slate-400"
              />
              <button className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-slate-400 hover:text-orange-600 rounded-md">
                <ScanBarcode size={20} />
              </button>
            </div>
            <button className="w-11 h-11 md:w-[46px] md:h-[46px] shrink-0 flex items-center justify-center bg-slate-100 rounded-xl text-slate-600 hover:text-orange-600 hover:bg-orange-50 transition-colors">
              <Filter size={20} />
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
          {products.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`bg-white rounded-2xl p-3 flex gap-3 transition-all ${inCart ? "border-2 border-orange-400 shadow-md ring-4 ring-orange-50" : "border border-slate-200 shadow-sm"}`}
              >
                {/* Ảnh & Nút Chi tiết */}
                <div className="w-24 md:w-28 shrink-0 flex flex-col gap-2">
                  <div className="w-24 h-24 md:w-28 md:h-28 bg-white rounded-xl border border-slate-100 overflow-hidden p-1.5 relative cursor-pointer group">
                    ...
                    />
                    <span className="absolute bottom-0 left-0 right-0 bg-slate-800/80 text-white text-[9px] font-bold text-center py-0.5 backdrop-blur-sm">
                      {p.unit}
                    </span>
                  </div>
                  <button
                    onClick={() => setSelectedProductDetail(p)}
                    className="w-full flex items-center justify-center gap-1 py-1.5 bg-orange-50/50 hover:bg-orange-50 text-orange-600 rounded-lg transition-colors border border-orange-200 hover:border-orange-300 active:scale-95"
                  >
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  <h3 className="text-[13px] font-bold text-slate-800 leading-snug line-clamp-2 mb-1">
                    {p.name}
                  </h3>
                  <div className="flex items-start gap-1 mb-1.5">
                    ...
                    />
                    <p className="text-[11px] text-slate-500 line-clamp-1 leading-tight font-medium">
                      {p.ingredient}
                    </p>
                  </div>
                  ...
                  {p.promo && (
                    <div className="mb-2 w-fit">
                      <span
                        className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold border ${p.promo.type === "gift" ? "bg-purple-50 text-purple-700 border-purple-200" : "bg-red-50 text-red-600 border-red-200"}`}
                      >
                        {p.promo.type === "gift" ? (
                          <Gift size={10} />
                        ) : (
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    {p.isLowStock ? (
                      <span className="text-[10px] text-red-600 font-bold flex items-center gap-0.5 bg-red-50 px-1 rounded">
                        <AlertTriangle size={10} /> Kho: {p.stock}
                      </span>
                    ) : (
                  ...
                  <div className="flex items-end justify-between mt-auto pt-1">
                    <div className="flex flex-col">
                      {inCart && (
                        <span className="text-[9px] font-bold text-orange-600 uppercase tracking-wider mb-0.5">
                          Đã chọn: {p.qty}
                        </span>
                      )}
                      <div className="text-[15px] font-black text-slate-900 leading-none">
                        {p.price.toLocaleString()}{" "}
                        <span className="text-xs font-semibold text-slate-500 underline decoration-slate-300 decoration-wavy">
                          đ
                        </span>
                      </div>
                    </div>
                    ...
                    {inCart ? (
                      <div className="flex items-center bg-white rounded-lg h-8 border-2 border-orange-400 shadow-sm overflow-hidden shrink-0">
                        ...
                        <input
                          ...
                          value={p.qty}
                          readOnly
                          className="w-10 h-full bg-orange-50 text-center text-sm font-black text-orange-700 outline-none border-x border-orange-100"
                        />
                        <button
                          onClick={() => updateQty(p.id, 1)}
                          className="w-8 h-full flex items-center justify-center text-orange-600 hover:bg-orange-50 active:bg-orange-100"
                      ...
                      <button
                        onClick={() => updateQty(p.id, 1)}
                        className="h-8 px-4 shrink-0 bg-white text-orange-600 font-black text-xs rounded-lg border border-slate-200 hover:border-orange-500 hover:text-orange-600 active:bg-orange-50 transition-all shadow-sm flex items-center gap-1"
                      >
        ...
        <div className="text-center pt-8 pb-12 flex items-center justify-center gap-2 text-slate-400">
          ...
          <p className="text-[11px] font-medium uppercase tracking-wider">
            Đã hiển thị toàn bộ 30 sản phẩm
          </p>
        </div>
      </main>
      ...
      <footer
        className={`fixed bottom-0 left-0 w-full transition-transform duration-300 z-40 ${totalItemCount > 0 ? "translate-y-0" : "translate-y-full"}`}
      >
        {/* VOUCHER HOOK (MOBILE) */}
        <div
        ...
        >
          <div className="flex items-center gap-2">
            <Ticket
              size={16}
              className={
                selectedVoucher ? "text-emerald-500" : "text-orange-500"
              }
            />
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 flex-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold">
                    {selectedVoucher
                      ...
                      : "Bạn có 2 mã ưu đãi. Áp dụng ngay!"}
                  </span>
                </div>
                {selectedVoucher ? (
              ...
              <div
                ...
                className="flex-1 md:flex-none relative cursor-pointer group p-2 md:p-3 md:bg-slate-50 md:rounded-2xl rounded-xl hover:bg-slate-100 transition-colors"
              >
          ...
          <div className="w-full md:max-w-md mx-auto md:mb-8 bg-slate-50 rounded-t-3xl md:rounded-3xl relative z-10 animate-in slide-in-from-bottom flex flex-col max-h-[85vh] shadow-2xl">
            ...
            <div className="p-4 border-b border-slate-200 bg-white rounded-t-3xl md:rounded-t-3xl shrink-0 flex justify-between items-center z-20">
              <div>
                ...
                <p className="text-[11px] font-medium text-slate-500">
                  Tổng cộng: {totalItemCount} mặt hàng
                </p>
              </div>
              <button
                onClick={() => setIsCartOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {cartItems.map((p) => (
                <div
                  key={`cart-${p.id}`}
                  className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex gap-3"
                >
                  <img
                    src={p.image}
                    alt={p.name}
                    className="w-16 h-16 rounded-lg border border-slate-100 object-contain p-1"
                  />
                  <div className="flex-1 flex flex-col justify-between min-w-0">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <button
                        ...
                        className="text-slate-300 hover:text-red-500 p-1 -m-1 transition-colors"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    <div className="flex justify-between items-end mt-2">
                      ...
                      <div className="flex items-center bg-slate-50 rounded-lg h-7 border border-slate-200 overflow-hidden shrink-0">
                        <button
                          onClick={() => updateQty(p.id, -1)}
                          className="w-7 h-full flex items-center justify-center text-slate-600 hover:bg-slate-200"
                        >
                          <Minus size={14} />
                        </button>
                        <input
                          ...
                          value={p.qty}
                          readOnly
                          className="w-8 h-full bg-transparent text-center text-xs font-bold text-slate-800 outline-none border-x border-slate-200"
                        />
                        <button
                          onClick={() => updateQty(p.id, 1)}
                          className="w-7 h-full flex items-center justify-center text-orange-600 hover:bg-orange-100"
                        >
                          <Plus size={14} />
                        </button>
                      </div>
                    ...
                    className={`relative w-full flex items-center p-4 rounded-xl border-2 transition-all ${
                      !isEligible
                        ...
                        : isSelected
                          ...
                          : "border-slate-200 bg-white hover:border-orange-200 cursor-pointer"
                    }`}
                  >
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 shrink-0 ${isSelected ? "bg-emerald-100 text-emerald-600" : "bg-orange-100 text-orange-600"}`}
                    >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      {!isEligible && (
                        <div className="text-[10px] text-red-500 font-semibold mt-1">
                          Mua thêm {(v.minOrder - totalAmount).toLocaleString()}
                          đ để dùng
                        </div>
                      )}
                    </div>
                    ...
                    {isEligible && (
                      <div
                        className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${isSelected ? "border-emerald-500 bg-emerald-500 text-white" : "border-slate-300"}`}
                      >
                        {isSelected && (
          ...
          <div className="w-full max-w-md bg-white rounded-[2rem] overflow-hidden relative z-10 animate-in zoom-in-95 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              <div className="mb-2 flex items-center gap-2">
                ...
                {selectedProductDetail.isLowStock && (
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className="text-2xl font-black text-orange-600">
                  {selectedProductDetail.price.toLocaleString()}đ
                </div>
                <div className="text-sm font-semibold text-slate-400 line-through">
                  {(selectedProductDetail.price * 1.1).toLocaleString()}đ
                </div>
              </div>
              ...
              {selectedProductDetail.promo && (
                <div className="mb-6 p-4 bg-gradient-to-r from-orange-50 to-red-50 border border-orange-100 rounded-2xl flex items-center gap-3">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-orange-500 shadow-sm shrink-0">
                    {selectedProductDetail.promo.type === "gift" ? (
                      <Gift size={20} />
                    ) : (
                      <Tag size={20} />
                    )}
                  ...
                  <div>
                    <div className="font-bold text-slate-800 text-sm">
                      {selectedProductDetail.promo.text}
                    </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2.5 border-b border-slate-100">
                  ...
                  <span className="text-sm text-slate-800 font-bold text-right max-w-[60%]">
                    {selectedProductDetail.ingredient}
                  </span>
                </div>
                <div className="flex justify-between py-2.5 border-b border-slate-100">
                  <span className="text-sm text-slate-500 font-medium">
                ...
                <div className="flex justify-between py-2.5 border-b border-slate-100">
                  ...
                  <span className="text-sm text-slate-800 font-bold">
                    ...
                    {selectedProductDetail.unit.toLowerCase()}
                  </span>
                </div>
                <div className="flex justify-between py-2.5 border-b border-slate-100">
                  <span className="text-sm text-slate-500 font-medium">
              ...
              {selectedProductDetail.qty > 0 ? (
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center bg-slate-50 rounded-xl h-[52px] border border-slate-200 overflow-hidden flex-1 max-w-[160px]">
                    <button
                      onClick={() => updateQty(selectedProductDetail.id, -1)}
                      className="w-12 h-full flex items-center justify-center text-slate-600 hover:bg-slate-200"
                    >
                      <Minus size={18} />
                    </button>
                    <input
                      type="number"
                      value={selectedProductDetail.qty}
                      readOnly
                      className="w-full h-full bg-transparent text-center text-base font-black text-slate-800 outline-none border-x border-slate-200"
                    />
                    <button
                      onClick={() => updateQty(selectedProductDetail.id, 1)}
                      className="w-12 h-full flex items-center justify-center text-orange-600 hover:bg-orange-100"
                    >
                      <Plus size={18} />
                    </button>
                  </div>
                  <button
                    onClick={() => setSelectedProductDetail(null)}
                    className="flex-1 h-[52px] bg-slate-800 hover:bg-slate-900 active:scale-95 text-white font-bold rounded-xl transition-all shadow-lg"
                  >
                    Xong
                  </button>
                </div>
                ...
                <button
                  onClick={() => updateQty(selectedProductDetail.id, 1)}
                  className="w-full h-[52px] bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 active:scale-95 text-white font-bold rounded-xl shadow-lg shadow-orange-200 transition-all flex items-center justify-center gap-2"
                >
